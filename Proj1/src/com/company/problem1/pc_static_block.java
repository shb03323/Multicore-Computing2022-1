...
public class pc_static_block {
    private static int NUM_END = 10000; // default input
    ...
    public static void main(String[] args) {
        ...
        int counter = 0;
        FindingThread[] ft = new FindingThread[NUM_THREADS];
        long programStartTime = System.currentTimeMillis();

        for (int i = 0; i < NUM_THREADS; i++) {
            ft[i] = new FindingThread((i * NUM_END) / NUM_THREADS, ((i + 1) * NUM_END) / NUM_THREADS);
            ft[i].start();
        }
...
class FindingThread extends Thread {
    ...
    long executionTime;
    int low, high;
    FindingThread (int low, int high) {
        this.low = low;
        this.high = high;
    }
